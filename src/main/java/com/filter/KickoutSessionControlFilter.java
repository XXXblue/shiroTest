...
public class KickoutSessionControlFilter  extends AccessControlFilter{
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        String prefix = "concurrent_person_cache_";
        RedisCache redisCache = (RedisCache) cache;
        // 初始化用户的队列放到缓存里
        Deque<Serializable> deque = null;
        deque = (Deque<Serializable>) redisCache.get(username,prefix);
        if(deque == null) {
            deque = new LinkedList<Serializable>();
            redisCache.put(username,deque,prefix);
        }
        ...
        if (session.getAttribute("kickout") != null) {
            ...
            responseNew.sendRedirect(requestNew.getContextPath()+"/login.jsp?kickout=1");
            return false;
        }
        return true;
    }
